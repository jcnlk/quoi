...
class ContainerTaskBuilder(val force: Boolean) {
    ...
    fun pickup(
        ...
        timeout: Int = 20,
        failureMessage: String = "Timed out",
    ) = click(slot, button, ClickType.PICKUP, timeout, failureMessage) // right/left click

    fun pickupAll(slot: MenuSlot) = click(slot, 0, ClickType.PICKUP_ALL) // double click

    fun throwOne(slot: MenuSlot) = click(slot, 0, ClickType.THROW) // q
    fun throwAll(slot: MenuSlot) = click(slot, 1, ClickType.THROW) // ctrl + q

    fun quickMove(target: MenuSlot) = click(target, 0, ClickType.QUICK_MOVE) // shift click
    fun swap(target: MenuSlot, hotbarSlot: Int) = click(target, hotbarSlot, ClickType.SWAP) // keys 1 to 9
