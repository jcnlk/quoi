...
interface ContainerAction { // todo cleanup
    ...
    /** Optional failure reason used by branch consumers that need structured feedback. */
    ...
    /** Cleans up resources owned by an action when its task is cancelled. */
    fun cancel() = Unit

    class Click(val slot: Int, val button: Int, val input: ClickType, val inContainer: Boolean?) : ContainerAction {
        ...
        override fun execute(): Boolean {
            ...
            if (!skipped) {
                gameMode.handleInventoryMouseClick(menu.containerId, s, button, input, player)
                activeTask?.ticksSinceLastClick = 0
            }
            activeTask?.skippedLast = skipped
            ContainerManager.finishFastBlockAfterClick()
    ...
    class DynamicClick(
        ...
        override val failureReason: String? get() = timeoutMessage.takeIf { failed }
        override var skipIf: ((ItemStack) -> Boolean)? = null
        override fun execute(): Boolean {
            val menu = player.containerMenu
            ...
            if (slot != null) {
                ...
                if (!skipped) {
                    gameMode.handleInventoryMouseClick(menu.containerId, slot, button, input, player)
                    activeTask?.ticksSinceLastClick = 0
                }
                activeTask?.skippedLast = skipped
                ContainerManager.finishFastBlockAfterClick()
    ...
    /** Runs after a click without clearing its skip state for a following [AwaitContainer]. */
    class AfterClick(val block: () -> Unit) : ContainerAction {
        override var skipIf: ((ItemStack) -> Boolean)? = null

        override fun execute(): Boolean {
            if (activeTask?.skippedLast != true) block()
            return true
        }
    }

    /** Fails the task with [failureMessage] when [predicate] is false. */
    class Check(
        ...
        override val abort: Boolean get() = failed
        override val failureReason: String? get() = failureMessage.takeIf { failed }
        override var skipIf: ((ItemStack) -> Boolean)? = null
    ...
    class AwaitContainer(
        ...
        override val abort: Boolean get() = failed
        override val failureReason: String? get() = failure
        override var skipIf: ((ItemStack) -> Boolean)? = null
        override fun execute(): Boolean {
            if (complete || failed) {
                cleanup()
            ...
            if (!started) {
                ...
                openSub = until<PacketEvent.Received, ClientboundOpenScreenPacket>(Priority.LOWEST) {
                    if (!containerName.containsMatchIn(packet.title.string)) {
                        ...
                        failure = "Wrong container name. Got ${packet.title.string}, needed ${containerName.pattern}"
                        failed = true
                        cleanup()
                        return@until false
                    }
                    ...
                    ContainerManager.beginFastBlock()
            ...
            if (ticksWaited >= timeout) {
                modMessage("Timed out")
                failure = "Timed out waiting for container matching ${containerName.pattern}"
                failed = true
                cleanup()
                return true
            }
