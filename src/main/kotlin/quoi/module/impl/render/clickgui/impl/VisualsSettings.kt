...
object VisualsSettings : SettingGroup(
    ...
    enum class ModuleSorting(val comparator: Comparator<Module>) {
        Longest(compareByDescending<Module> { UIRenderer.textWidth(it.name, 18f, UIRenderer.defaultFont) }.thenBy { it.name.lowercase() }),
        Shortest(compareBy<Module> { UIRenderer.textWidth(it.name, 18f, UIRenderer.defaultFont) }.thenBy { it.name.lowercase() }),
