...
object BloodCamp : Module( // todo auto
    "Blood Camp",
    area = Island.Dungeon(inClear = true)
) {
    private val colour by colourPicker("Colour", Colour.RED)
    private val tickOffset by slider("Spawn offset", 0, -20, 20, unit = "t")

    private var watcherPhase = 0
    private val entities = ConcurrentHashMap<Int, BloodEntity>()
    ...
    init {
        on<PacketEvent.Received, ClientboundMoveEntityPacket> {
            ...
            val texture = headItem.texture ?: return@on
            if (texture !in textures) return@on
            ...
            val bloodEntity = entities.getOrPut(entity.id) {
                BloodEntity(
                    ...
                    vecs = mutableListOf(),
                    ticks = tickOffset + if (watcherPhase != 0) 19 else 39
                )
            }
            ...
            val scaleFactor = (1.0 / bloodEntity.vecs.size) * if (bloodEntity.ticks < 20) 13.5 else 26.5
            vec = vec.scale(scaleFactor)

            bloodEntity.finalPos = bloodEntity.initPos.add(vec)
        }

        on<ChatEvent.Packet> {
            if (BLOOD_START_REGEX.matches(unformatted)) {
                watcherPhase = 0
            } else if (unformatted == "[BOSS] The Watcher: Let's see how you can handle this.") {
                watcherPhase = 1
            }
        }
        ...
        on<RenderEvent.World> {
            if (currentRoom?.data?.type != RoomType.BLOOD) return@on
            entities.values.removeIf { !it.entity.isAlive }

            for ((_, v) in entities) {
                val aabb = AABB(0.75, 0.75, 0.75, 0.0, 0.0, 0.0).move(
                    v.finalPos.x - 0.375,
                    v.finalPos.y + 1.0,
                    v.finalPos.z - 0.375
                )
                ctx.drawWireFrameBox(aabb, colour)

                val vec1 = v.finalPos.subtract(v.entity.position())
                val vec2 = v.finalPos.subtract(v.initPos)
                val len2 = vec2.length()

                val progress = if (len2 > 0) vec1.length() / len2 else 0.0
                val time = literal((progress * v.ticks / 20.0).toFixed() + "s")

                ctx.drawText(time, v.finalPos.add(0.0, 1.5, 0.0), scale = 1.5f, shadow = true)
            }
        }
    ...
    private data class BloodEntity(val entity: ArmorStand, val initPos: Vec3, var finalPos: Vec3, val vecs: MutableList<Vec3>, val ticks: Int)
