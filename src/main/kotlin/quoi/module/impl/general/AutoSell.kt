...
object AutoSell : Module(
    ...
    private val normalizedEntries get() = sellList.map(::normalizeSellEntry).filter(String::isNotEmpty).distinct()

    init {
        val autoSellCommand = command.sub("autosell").description("Auto Sell module settings.")

        autoSellCommand.sub("add") { item: GreedyString? ->
            setSellEntry(item?.string ?: player.mainHandItem.sellListName(), add = true)
        }.description("Adds an item to the auto sell list.")

        autoSellCommand.sub("remove") { item: GreedyString? ->
            setSellEntry(item?.string ?: player.mainHandItem.sellListName(), add = false)
        ...
        autoSellCommand.sub("list") {
            ...
            val chunkedList = normalizedEntries.chunked(10)
        ...
        on<GuiEvent.Key.Press> {
            ...
            setSellEntry(name, add = name !in normalizedEntries)
        }

        on<TickEvent.Start> {
            if (sellList.isEmpty()) return@on
            val menu = (mc.gui.screen() as? AbstractContainerScreen<*>)?.menu ?: return@on
            if (!menu.isSellMenu()) return@on

            val now = System.currentTimeMillis()
            if (now < nextClick) return@on

            val entries = normalizedEntries
            val candidates = menu.slots.filter { it.container is Inventory && shouldSell(it.item, entries) }
            val slot = candidates.firstOrNull { it.index > lastSlot } ?: candidates.firstOrNull() ?: return@on
            lastSlot = slot.index
    ...
    private fun shouldSell(stack: ItemStack, entries: List<String>): Boolean {
        val itemName = stack.sellListName() ?: return false
        return "skeleton master chestplate" !in itemName && entries.any(itemName::contains)
    }

    private fun setSellEntry(value: String?, add: Boolean) {
        val name = value?.let(::normalizeSellEntry)?.takeIf(String::isNotEmpty)
            ?: return modMessage("Either hold an item or write an item name for autosell.")
        val changed = if (add) name !in normalizedEntries && sellList.add(name)
            else sellList.removeAll { normalizeSellEntry(it) == name }
