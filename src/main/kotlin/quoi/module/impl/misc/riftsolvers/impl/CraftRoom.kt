...
object CraftRoom : ToggleableGroup(MirrorverseSolvers, "Craft room") {
    ...
    init {
        on<RenderEvent.World> {
            if (!mobs) return@on
            if (player.z > -100.0 && player.z < -140.0) return@on
        ...
        on<GuiEvent.DrawTooltip> {
            if (!recipe) return@on
            if (!player.boundingBox.intersects(craftRoomArea)) return@on
    ...
    private fun GuiGraphics.drawRecipe(x: Int, y: Int, recipe: Array<ItemStack?>) {
        ...
        for (row in 0..2) {
            for (col in 0..2) {
                ...
                renderItem(item, x, y)
                renderItemDecorations(QuoiMod.mc.font, item, x, y)
            }
        }
    }
    ...
    private val stick by lazy { ItemStack(Items.STICK) }
    private val string by lazy { ItemStack(Items.STRING) }
    private val slime by lazy { ItemStack(Items.SLIME_BALL) }

    private val rod by lazy { ItemStack(Items.FISHING_ROD) }
    private val bow by lazy { ItemStack(Items.BOW) }
    private val lead by lazy { ItemStack(Items.LEAD) }

    private val rodRecipe by lazy {
        arrayOf(
            stick,  null,  null,
            string, stick, null,
            string, null,  stick
    ...
    private val leadRecipe by lazy {
        arrayOf(
            null,   string, string,
            null,   slime,  string,
            string, null,   null
    ...
    private val hammerRecipe by lazy {
        arrayOf(
            lead, bow, null,
            bow,  rod, null,
            null, rod, null
